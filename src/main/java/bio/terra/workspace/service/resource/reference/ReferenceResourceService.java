...
public class ReferenceResourceService {
    ...
    @Autowired
    public ReferenceResourceService(JobService jobService, ResourceDao resourceDao, SamService samService) {
        this.jobService = jobService;
        this.resourceDao = resourceDao;
        this.samService = samService;
    }

    public ReferenceResource createReferenceResource(ReferenceResource resource, AuthenticatedUserRequest userReq) {
        String jobDescription = String.format("Create reference %s; id %s; name %s",
                 resource.getResourceType(), resource.getResourceId(),
                resource.getName());

        JobBuilder createJob = jobService.newJob(
                                jobDescription,
                                UUID.randomUUID().toString(),
                                CreateReferenceResourceFlight.class,
                                resource,
                                userReq)
                .addParameter(WorkspaceFlightMapKeys.ResourceKeys.RESOURCE_TYPE, resource.getResourceType());
    ...
    /**
     * At this moment, the only updates on references we are doing are to name and description
     * so this is a common implementation. If we get more complicated, we can break it out.
    ...
    /**
     * Delete a reference. The only state we hold for a reference is in the metadata database
     * so we directly delete that.
     *
     * @param workspaceId workspace of interest
    ...
    public ReferenceResource getReferenceResource(UUID workspaceId, UUID resourceId, AuthenticatedUserRequest userReq) {
        WsmResource wsmResource = resourceDao.getResource(workspaceId, resourceId);
        return castReferenceResource(wsmResource);
    }

    public ReferenceResource getReferenceResourceByName(UUID workspaceId, String name, AuthenticatedUserRequest userReq) {
