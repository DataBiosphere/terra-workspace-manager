...
start() {
    ...
    BASEDIR=$(dirname "$0")
    # TODO(): Delete max_connections after unit tests stop leaking connections
    docker create --name $CONTAINER --rm -e POSTGRES_PASSWORD=password -p "$POSTGRES_PORT:5432" postgres:$POSTGRES_VERSION -c max_connections=150
