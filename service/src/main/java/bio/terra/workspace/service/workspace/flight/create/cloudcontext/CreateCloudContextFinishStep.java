 ...
 * <p>Optional CLOUD_CONTEXT_STRING - serialized string of cloud-specific data stored with the cloud context row
 * <p>Required CLOUD_CONTEXT_HOLDER - the cloud context holder filled in with the cloud-specific context object.
 */
...
public class CreateCloudContextFinishStep implements Step {
  ...
  public CreateCloudContextFinishStep(UUID workspaceUuid, WorkspaceDao workspaceDao, CloudPlatform cloudPlatform) {
  ...
  public StepResult doStep(FlightContext flightContext)
    ...
    CloudContext cloudContext = FlightUtils.getRequired(workingMap, WorkspaceFlightMapKeys.CLOUD_CONTEXT, CloudContext.class);
    String contextJson = cloudContext.serialize();
    workspaceDao.createCloudContextSuccess(workspaceUuid, cloudPlatform, contextJson, flightContext.getFlightId());
    ...
    CloudContext fullCloudContext =
      dbCloudContext.getCloudPlatform().getCloudContextService().makeCloudContextFromDb(dbCloudContext);
