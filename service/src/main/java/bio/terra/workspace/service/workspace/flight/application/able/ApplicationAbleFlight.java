...
/**
 * This flight handles both enabling and disabling applications on workspaces. It
 * is structured so that if there is disagreement between Sam and our database,
 * re-running the flight will fix that. We run a "precheck" step to learn the current state
 * and make sure we don't undo things that were there before we started. Then we apply
 * the Sam change and the database change.
 */
public class ApplicationAbleFlight extends Flight {
  public ApplicationAbleFlight(FlightMap inputParameters, Object applicationContext) {
    ...
    UUID workspaceId =
        inputParameters.get(WorkspaceFlightMapKeys.WORKSPACE_ID, UUID.class);
    UUID applicationId =
        inputParameters.get(WorkspaceFlightMapKeys.APPLICATION_ID, UUID.class);
    AbleEnum ableEnum =
    ...
    addStep(new ApplicationAblePrecheckStep(
    ...
    addStep(new ApplicationAbleIamStep(
        beanBag.getSamService(),
        workspaceId,
        ableEnum));

    // ApplicationEnableDaoStep - On do, if application did not already have the application enabled,
    // enable it. On undo, if application did not already have the application enabled, disable it.
    // set the result
    addStep(new ApplicationAbleDaoStep(
        beanBag.getApplicationDao(),
        workspaceId,
        applicationId,
        ableEnum));
