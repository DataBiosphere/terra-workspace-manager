...
/**
 * Make the destination controlled GCS bucket resource object and the associated
 * resource parameters and store them in the working map.
 */
public class MakeControlledGcsBucketResourceForCloneStep implements Step {
  ...
  public StepResult doStep(FlightContext flightContext)
    ...
    var sourceParameters = workingMap.get(ControlledResourceKeys.RESOURCE_PARAMETERS, ControlledGcsBucketParameters.class);
    ...
    destinationParameters.setLocation(
      Optional.ofNullable(destinationParameters.getLocation()).orElse(sourceParameters.getLocation()));
    workingMap.put(ControlledResourceKeys.RESOURCE_PARAMETERS, destinationParameters);

    // Construct the destination resource
    ControlledGcsBucketResource destinationResource = ControlledGcsBucketResource.builder()
