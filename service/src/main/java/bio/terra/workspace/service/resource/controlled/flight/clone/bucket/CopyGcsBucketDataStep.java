...
public class CopyGcsBucketDataStep implements Step {
  ...
  // See https://cloud.google.com/storage-transfer/docs/reference/rest/v1/transferJobs/create (somewhat dated) and
  ...
  public StepResult doStep(FlightContext flightContext)
    ...
    final String destinationBucketName = flightContext.getWorkingMap().get(ControlledResourceKeys.DESTINATION_BUCKET_NAME, String.class);
    try {
      final Storagetransfer storageTransferService = createStorageTransferService();
      final TransferSpec transferSpec = new TransferSpec()
          ...
          .setTransferOptions(new TransferOptions().setDeleteObjectsFromSourceAfterTransfer(false).setOverwriteObjectsAlreadyExistingInSink(true))
      final String jobName = createTransferJobName();
      final String projectId = null; // FIXME
      final TransferJob transferJobInput = new TransferJob()
          .setName(jobName)
      ...
      // Job is now submitted with its schedule. We need to Poll for completion of the first transfer.
//      boolean isDone = false;
//      do {
//        final Storagetransfer.TransferJobs.Get getRequest = storageTransferService
//            .transferJobs()
//            .get(jobName, projectId);
//        final TransferJob getResponse = getRequest.execute();
//        if (getResponse.getLatestOperationName())
//      } while (!isDone);
  ...
  private Schedule createScheduleRunOnceNow() {
    ...
    final TimeOfDay runTimeOfDay = new TimeOfDay()
  ...
  private Storagetransfer createStorageTransferService()
    ...
    return new Storagetransfer.Builder(Utils.getDefaultTransport(),
