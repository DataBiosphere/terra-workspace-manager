...
/**
 * Generate flight IDs and a workspace ID or two ahead of the steps that need them.
 * The steps can then check the existence of the flights or workspaces before creation
...
public class CreateIdsForFutureStepsStep implements Step {
  ...
  public StepResult doStep(FlightContext context) throws InterruptedException, RetryException {
    ...
    workingMap.put(ControlledResourceKeys.CREATE_CLOUD_CONTEXT_JOB_ID,
        context.getStairway().createFlightId());
    workingMap
        .put(ControlledResourceKeys.DESTINATION_WORKSPACE_ID, UUID.randomUUID());
    workingMap
        .put(ControlledResourceKeys.WORKSPACE_CREATE_JOB_ID, context.getStairway().createFlightId());
