...
def get_notebook_cred(session, workspace_id, notebook_id, access):
    response = session.get(f'{WSM_API_ENDPOINT}/{workspace_id}/resources/controlled/aws/sagemaker-notebooks/{notebook_id}/credential',
        params={'accessScope': access, 'credentialDuration': 900})
    if not response.ok:
