...
public class FlightGenerator extends Flight {
  ...
  protected <T> T createStep(T step) {
    ...
    var stepProxy = Proxy.newProxyInstance(
  ...
  protected <T> void setResponse(T response) {
    if (Proxy.isProxyClass(response.getClass()) && Proxy.getInvocationHandler(response) instanceof StepResultInvocationHandler) {
      var responseStepIndex = ((StepResultInvocationHandler) Proxy.getInvocationHandler(response)).stepIndex();
