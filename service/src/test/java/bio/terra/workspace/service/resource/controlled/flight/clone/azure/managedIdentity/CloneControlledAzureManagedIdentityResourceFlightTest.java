...
public class CloneControlledAzureManagedIdentityResourceFlightTest extends BaseAzureConnectedTest {
    ...
    public void cleanup() {
        ...
        workspaceService.deleteWorkspace(destinationWorkspace, userAccessUtils.defaultUserAuthRequest());
    }

    @Test
    void cloneControlledAzureManagedIdentity()
        ...
        var sourceResource =
                ControlledAzureResourceFixtures.makeDefaultControlledAzureManagedIdentityResourceBuilder(creationParameters, sourceWorkspace.workspaceId())
                .build();
        ...
        inputs.put(WorkspaceFlightMapKeys.ResourceKeys.CLONING_INSTRUCTIONS, CloningInstructions.COPY_DEFINITION.name());
        inputs.put(
                WorkspaceFlightMapKeys.ControlledResourceKeys.DESTINATION_RESOURCE_ID, destinationResourceId);
        inputs.put(
                WorkspaceFlightMapKeys.ControlledResourceKeys.DESTINATION_RESOURCE_NAME, sourceResource.getName());
        inputs.put(
                WorkspaceFlightMapKeys.ControlledResourceKeys.DESTINATION_WORKSPACE_ID, destinationWorkspace.workspaceId());
        ...
        assertEquals(resultIdentity.effectiveCloningInstructions(), CloningInstructions.COPY_DEFINITION);
        assertEquals(resultIdentity.managedIdentity().getWorkspaceId(), destinationWorkspace.workspaceId());
        ...
        assertNotEquals(resultIdentity.managedIdentity().getManagedIdentityName(), sourceResource.getManagedIdentityName());
        assertEquals(result.getFlightStatus(), FlightStatus.SUCCESS);

        ControlledAzureManagedIdentityResource resourceInWorkspace = controlledResourceService.getControlledResource(destinationWorkspace.workspaceId(), destinationResourceId).castByEnum(WsmResourceType.CONTROLLED_AZURE_MANAGED_IDENTITY);
        assertNotNull(resourceInWorkspace);
        assertEquals(resultIdentity.managedIdentity().attributesToJson(), resourceInWorkspace.attributesToJson());
        assertEquals(resultIdentity.managedIdentity().getManagedIdentityName(), resourceInWorkspace.getManagedIdentityName());
        assertEquals(resultIdentity.managedIdentity().getWsmControlledResourceFields(), resourceInWorkspace.getWsmControlledResourceFields());
