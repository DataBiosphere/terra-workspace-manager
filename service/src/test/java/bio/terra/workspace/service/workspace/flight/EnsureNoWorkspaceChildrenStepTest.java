...
public class EnsureNoWorkspaceChildrenStepTest {
  ...
  void stepPassesWhenNoChildrenExistForWorkspace() throws Exception {
    ...
    var result = new EnsureNoWorkspaceChildrenStep(sam, request, workspaceId).doStep(Mockito.mock(FlightContext.class));
  ...
  void stepFailsWhenChildrenExistForWorkspace() throws Exception {
    ...
    var children = List.of(new FullyQualifiedResourceId().resourceId(resourceId).resourceTypeName(resourceType));
    when(sam.getWorkspaceChildResources(request, workspaceId)).thenReturn(children);

    var e = assertThrows(
        ChildrenBlockingDeletionException.class,
        () -> new EnsureNoWorkspaceChildrenStep(sam, request, workspaceId).doStep(Mockito.mock(FlightContext.class))
    );
