...
class DetachAzureDiskFromVmStepUnitTest {
    ...
    @Mock
    private AzureConfiguration azureConfigurationMock;
    @Mock
    private CrlService crlServiceMock;
    @Mock private ControlledAzureDiskResource controlledAzureDiskResourceMock;
    @Mock
    private FlightContext flightContextMock;
    @Mock
    private AzureCloudContext azureCloudContextMock;
    @Mock
    private FlightMap flightWorkingMapMock;
    @Mock
    private ComputeManager computeManagerMock;
    @Mock
    private Disks disksMock;
    @Mock
    private Disk diskMock;
    @Mock
    private VirtualMachines virtualMachinesMock;
    @Mock
    private VirtualMachine virtualMachineMock;
    @Mock
    private VirtualMachineDataDisk virtualMachineDataDiskMock;
    @Mock
    private VirtualMachine.Update virtualMachineUpdateMock;

    private DetachAzureDiskFromVmStep detachAzureDiskFromVmStep;

    @BeforeEach
    void setup() {
        detachAzureDiskFromVmStep = new DetachAzureDiskFromVmStep(azureConfigurationMock, crlServiceMock, controlledAzureDiskResourceMock);
    ...
    void doStep_DiskIsAttached() throws InterruptedException {
        final Map<Integer, VirtualMachineDataDisk> dataDiskMap = Map.of(diskLun, virtualMachineDataDiskMock);
        setupCommonMocks();
    ...
    private void setupAzureMocksForDoStep(boolean isDiskAttachedToVm, boolean isVmFound, boolean diskMatchesExistingVmDisks, Map<Integer, VirtualMachineDataDisk> dataDiskMap, String vmId) {
        ...
        if (isDiskAttachedToVm) {
            ...
            if (isVmFound) {
                ...
                if (diskMatchesExistingVmDisks) {
                    ...
                    when(virtualMachineUpdateMock.withoutDataDisk(anyInt())).thenReturn(virtualMachineUpdateMock);
                }
            } else {
                when(computeManagerMock.virtualMachines()).thenReturn(virtualMachinesMock);
    ...
    private void setupAzureMocksForUndoStep(boolean diskWasInitiallyAttached) {
        ...
        when(computeManagerMock.disks()).thenReturn(disksMock);
        when(flightWorkingMapMock.get(
                DeleteAzureDiskFlightUtils.DISK_ATTACHED_VM_ID_KEY,
                String.class)).thenReturn(diskWasInitiallyAttached? vmId : null);
        when(disksMock.getById(anyString())).thenReturn(diskMock);
    }
