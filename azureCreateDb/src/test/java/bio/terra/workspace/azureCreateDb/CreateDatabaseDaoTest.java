...
public class CreateDatabaseDaoTest extends BaseUnitTest {
  ...
  void testCreateDatabase() {
    ...
    jdbcTemplate.query("select count(*) from pg_database where datname = lower(?)", new Object[]{testDatabaseName}, new int[]{
            Types.VARCHAR}, (rs, rowNum) -> rs.getInt(1))
  ...
  /**
   * Creates a function the is expected to exist in an Azure Postgres database. The real
   * thing does more than just creates a role.
   */
  private void createRoleFunction() {
    var function = "create or replace function pgaadauth_create_principal_with_oid(rolename text, oid text, foo text, bar boolean, baz boolean)"
        + " returns text"
        + " language plpgsql"
        + " as"
        + " $$"
