...
TESTRUNNER_IAM_SERVICE_ACCOUNT_VAULT_PATH=secret/dsde/terra/kernel/integration/ichang/testrunner-sa
TESTRUNNER_IAM_SERVICE_ACCOUNT_OUTPUT_PATH=$(dirname "$0")/rendered/testrunner-iam-service-account.json
...
docker run --rm -e VAULT_TOKEN="$VAULT_TOKEN" $DSDE_TOOLBOX_DOCKER_IMAGE \
    vault read -format=json ${TESTRUNNER_IAM_SERVICE_ACCOUNT_VAULT_PATH} | \
    jq -r .data.key | base64 --decode > ${TESTRUNNER_IAM_SERVICE_ACCOUNT_OUTPUT_PATH}
